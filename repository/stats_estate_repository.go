...
type StatsEstateRepository interface {
	QueryAllTree(estateId string) ([]model.Tree, error)
	SaveStatsEstate(estateStats model.EstateStats) error
	QueryById(estateId string) (*model.EstateStats, error)
}
...
func (s *statsEstateRepository) QueryById(estateId string) (*model.EstateStats, error) {
	var statsEstate model.EstateStats
	err := s.DB.QueryRowContext(context.Background(), "SELECT estate_id, min_height_tree, max_height_tree, median_height_tree, sum_tree, total_distance_drone, ext_info,created_at, updated_at  FROM estate_stats WHERE estate_id = $1", estateId).
	...
	if errors.Is(err, sql.ErrNoRows) {
		...
		log.Println("Failed to query block by estateId")
...
func (s *statsEstateRepository) SaveStatsEstate(estateStats model.EstateStats) error {
	_, err := s.DB.ExecContext(context.Background(), `INSERT INTO estate_stats (
    estate_id, 
    sum_tree, 
    min_height_tree, 
    max_height_tree, 
...
func (s statsEstateRepository) QueryAllTree(estateId string) ([]model.Tree, error) {
